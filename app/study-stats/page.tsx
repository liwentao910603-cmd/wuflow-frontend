    ...
    <div style={{ overflowX: "auto" }}>
      <div style={{ position: "relative", paddingTop: 20, display: "inline-block" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 8, justifyContent: "flex-end" }}>
          ...
          {["#f0f0f0", "#c7d2fe", "#818cf8", "#6366f1", "#4338ca"].map((c, i) => (
      ...
      <main className="flex-1 overflow-y-auto" style={{ background: "#f9f9f8" }}>
        <div style={{ maxWidth: 860, margin: "0 auto", padding: "48px 32px" }}>
          ...
          <div style={{ marginBottom: 36 }}>
            <h1 style={{ fontSize: 26, fontWeight: 600, color: "#111", margin: "0 0 6px", letterSpacing: "-0.5px" }}>
              学习统计 📊
            </h1>
            ...
            {statCards.map((s, i) => (
              <div key={i} style={{ background: "#fff", border: "1px solid #ebebeb", borderRadius: 14, padding: "20px 22px" }}>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 4 }}>
                  <span style={{ fontSize: 28, fontWeight: 600, color: s.value === 0 ? "#ddd" : "#111", letterSpacing: "-1px" }}>
                    {s.value}
                  </span>
          ...
          <div style={{ background: "#fff", border: "1px solid #ebebeb", borderRadius: 14, padding: "24px 28px", marginBottom: 24 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#999", marginBottom: 16, letterSpacing: "0.5px", textTransform: "uppercase" }}>
              过去 90 天学习热力图
            </div>
            {loading ? (
              <div style={{ height: 100, display: "flex", alignItems: "center", justifyContent: "center", color: "#ccc", fontSize: 13 }}>加载中...</div>
            ) : (
          ...
          <div style={{ background: "#fff", border: "1px solid #ebebeb", borderRadius: 14, padding: "24px 28px" }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#999", marginBottom: 20, letterSpacing: "0.5px", textTransform: "uppercase" }}>
              打卡历史
            </div>
            {loading ? (
              <div style={{ textAlign: "center", padding: "40px 0", color: "#ccc", fontSize: 13 }}>加载中...</div>
            ) : logs.length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 0" }}>
                <div style={{ fontSize: 32, marginBottom: 12 }}>📝</div>
                <div style={{ fontSize: 14, color: "#bbb" }}>还没有打卡记录</div>
                <div style={{ fontSize: 13, color: "#ccc", marginTop: 6 }}>回到主页完成第一次打卡</div>
              </div>
            ) : (
                ...
                {logs.map((log, i) => (
                  <div key={log.id}>
                    {(i === 0 || logs[i - 1].logged_date !== log.logged_date) && (
                      <div style={{ fontSize: 11, fontWeight: 600, color: "#bbb", padding: "12px 0 8px", letterSpacing: "0.5px" }}>
                        {formatDate(log.logged_date)}
                      </div>
                    )}
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "12px 0", borderBottom: "1px solid #f5f5f5" }}>
                      <div style={{ fontSize: 22, lineHeight: 1, marginTop: 2 }}>{MOOD_EMOJI[log.mood]}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ fontSize: 14, color: "#333", margin: "0 0 6px", lineHeight: 1.6 }}>{log.content}</p>
                        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                          <span style={{ fontSize: 12, color: "#bbb" }}>⏱ {log.duration_minutes} 分钟</span>
                          <span style={{ fontSize: 12, color: "#bbb" }}>状态：{MOOD_LABEL[log.mood]}</span>
                        </div>
                      </div>
                    </div>
                  </div>
